...
[ExecuteInEditMode]
public class SpriteMaskTrueSoft : SpriteMaskingComponent
{
	...
	/// <summary>
	/// Forces to use this object default material on all his childs. This is useful when moving sprite between different SpriteMask objects.
	/// </summary>
	public bool forceDefaultMaterialOnChilds = false;

	/// <summary>
	/// Forces to use individual material on all his childs. This is useful when you have many objects (masked and unmasked) with the same material.
	/// </summary>
	public bool forceIndividualMaterialOnChilds = false;
	...
	private static List<SpriteMaskTrueSoft> maskComponents = new List<SpriteMaskTrueSoft> ();
...
//		}
	...
	/// </summary>
	public static SpriteMaskTrueSoft getParentMask (Transform t)
	{
		t = t.parent;
		while (t != null) {
			SpriteMaskingComponent masking = getMaskingComponent (t);
...
//							if (ms [i] == null || !hasStencilSupport (ms [i]) || forceDefaultMaterialOnChilds) {
	...
	private Material getIndividualMaterial (Material origin)
	{
		...
		if (individualMaterials.ContainsKey (origin)) {
			...
			Material m = new Material (origin);
			m.name += " (Clone)";
			clearMaterial (origin);
			individualMaterials [origin] = m;
			return m;
		}
	}
	...
	public Vector2 getPivot (Bounds bounds)
	{
		...
		pivot.x = -bounds.center.x / bounds.extents.x / 2 + 0.5f;
		pivot.y = -bounds.center.y / bounds.extents.y / 2 + 0.5f;
		return pivot;
	}
